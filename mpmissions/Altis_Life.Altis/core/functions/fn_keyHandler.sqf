...
{	
	...
	case 20:
	{
		...
		{	
			if(life_koffSpam) exitWith {hint "Nicht spamen...";};
			life_koffSpam = true;
			...
			[] spawn {sleep 2; life_koffSpam = false};
		};
   ...
   case 24:
   {
	...
	{
		if(_shift) then {_handled = true;};
		if(_shift && !life_PB && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then
		{
		...
		if(_shift && playerSide == west && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then
		{
			if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player OR currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable["restrained",false]) && !life_istazed) then
			{
				[cursorTarget] spawn life_fnc_knockoutAction; // We got to add an extra 
			};
		};
	};
...
/* 	//Ergeben(Shift + G)
	...
	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F","I_MRAP_03_hmg_F","I_MRAP_03_F","B_APC_Wheeled_01_cannon_F","B_MRAP_01_hmg_F","C_Hatchback_01_F","C_Hatchback_01_sport_F","B_Quadbike_01_F"]) then {
		...
		if(_shift && (playerSide == east)) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F"]) then {
				if(!isNil {vehicle player getVariable "lights"}) then {
					[vehicle player] call life_fnc_serviceLights;
					_handled = true;
				};
			};
		};
	...
	case 201:
	{
		if(_shift) then
		{
			...
			if (playerside == west) then
			{
				switch (player getVariable["life_Earplugs",0]) do {
					case 0: {titleText ["Ear Plugs 90%", "PLAIN"]; 1 fadeSound 0.1; player setVariable ["life_Earplugs", 10]; };
					case 10: {titleText ["Ear Plugs 60%", "PLAIN"]; 1 fadeSound 0.4; player setVariable ["life_Earplugs", 40]; };
					case 40: {titleText ["Ear Plugs 30%", "PLAIN"]; 1 fadeSound 0.7; player setVariable ["life_Earplugs", 70]; };
					case 70: {titleText ["Ear Plugs Removed", "PLAIN"]; 1 fadeSound 1; player setVariable ["life_Earplugs", 0]; };
				};
			} else {
			   ...
			   {
					1 fadeSound 0.2;
					titleText ["Dein Sound wurde verringert.", "PLAIN"];
			   };
		   };
		};
	};
